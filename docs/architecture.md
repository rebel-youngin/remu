...
- **`remu-shm`** (36 GB by default — `R100_RBLN_DRAM_SIZE`, matches
  real CR03 silicon's per-PF DRAM window) — `r100-npu-pci.memdev`
  over BAR0 offset 0 on the host; `r100-soc.memdev` over chiplet-0
  DRAM offset 0 on the NPU. Layout per chiplet, as the kmd allocates
  it: 1 GB `CP_SYSTEM_SIZE` system region (FW images / MMU
  page-table pool / sync block) + 35 GB user region (kmd's DVA pool
  for umd buffers); aggregate device memory across 4 CR03 chiplets
  is 144 GB but only chiplet 0 is PCIe-exposed via BAR0 — chiplets
  1..3 stay NPU-private. Tmpfs is sparse so the 36 GB ftruncate
  only reserves address range; the hard requirement is `/dev/shm`
  with ≥ 36 GB free. Override with `./remucli run --shm-size
  <bytes>`; the lazy `bar0_tail` / `dram_tail` paths in
  `r100_npu_pci.c` / `r100_soc.c` keep BL31_CP1 / FreeRTOS_CP1 at
  `0x14100000` / `0x14200000` reachable for sub-default
  `--shm-size`. (No security boundary: REMU is functional, the host
  x86 is just another vCPU.)
- **`host-ram`** (`--host-mem`, default 512 MB) — host x86 QEMU's
  main RAM `memory-backend-file`. The NPU's `r100-pcie-outbound`
  aliases the same backend over the chiplet-0 PCIe AXI window at
  `R100_PCIE_AXI_SLV_BASE_ADDR = 0x8000000000`, so q-cp's outbound
  iATU loads/stores hit the same pages the kmd allocates with
  `dma_alloc_coherent` — no chardev hop, no BQL contention with
  the kmd's `readl_poll_timeout_atomic`.
- **`cfg-shadow`** (4 KB) — host x86 QEMU's BAR2 cfg-head subregion
  at `FW_LOGBUF_SIZE` aliases this backend; NPU `r100-cm7`
  cfg-mirror trap at `R100_DEVICE_COMM_SPACE_BASE = 0x10200000`
  aliases the same backend. Result: kmd writes to `FUNC_SCRATCH` /
  `DDH_BASE_LO` are visible to q-cp's next read with no chardev
  queue and no ordering race against the doorbell, and q-cp's
  `cb_complete → writel(FUNC_SCRATCH, magic)` lands in the shared
  backend and is observable on the kmd's next `rebel_cfg_read`.

HMP monitors: NPU on `output/<name>/npu/monitor.sock` (plus the
stdio-muxed readline monitor on uart0), host on
`output/<name>/host/monitor.sock`.

### x86 Linux guest (M8b Stage 2)

With `images/x86_guest/{bzImage,initramfs.cpio.gz}` staged (via
`./guest/build-guest-image.sh`), `--host` auto-adds `-kernel/-initrd`
+ virtio-9p share of `guest/` at `/mnt/remu` + `-cpu max` (stock kmd
is `-march=native` with BMI2 → `#UD` on minimal `qemu64`). The guest
...
`--no-guest-boot`, `--guest-cmdline-extra`. By default SeaBIOS runs
and the x86 guest idles at "No bootable device", serial to
`host/serial.log`.
...
| `r100_smmu.{c,h}` | SMMU-600 TCU | 1 / chiplet | **MMIO** (128 KB region — page 0 SMMU-600 control block, page 1 `EVENTQ_PROD/CONS` at offset `0x100A8/AC`): `CR0→CR0ACK` + `GBPA.UPDATE` auto-clear; `STRTAB_BASE` / `STRTAB_BASE_CFG` / `EVENTQ_BASE` / `EVENTQ_PROD` / `EVENTQ_CONS` / `IRQ_CTRL.{EVENTQ,GERROR}_IRQEN` (mirrored into `IRQ_CTRLACK`) / `GERROR` / `GERRORN` cache the in-DRAM queue geometry and gate the fault-delivery path; `CMDQ_PROD` walks `(old_cons, new_prod]` and recognises `CMD_SYNC` + the full `CMD_TLBI_*` / `CMD_CFGI_*` / `CMD_PREFETCH_*` opcode set (logged, advance CONS as no-ops — no STE / IOTLB cache to invalidate, every translate re-reads STE). **Public translate API** (`r100_smmu_translate`): NPU-side engines that consume DVAs call this before `address_space_*`. Pre-`CR0.SMMUEN`: identity. Post-enable: read STE for `sid` from `STRTAB_BASE_PA + sid * 64`, decode `STE0.{V, config}` (`BYPASS`→identity, `ABORT`→`INV_STE` fault, `S1_TRANS`→stage-1 walk: read `STE1.S1DSS` to gate (`SUBSTREAM0`→walk `CD[0]` from `STE0.S1ContextPtr` and dispatch QEMU's `smmu_ptw_64_s1` against the FW's bypass PT; `BYPASS`→identity matching FW's `smmu_init_ste` regular SIDs; `TERMINATE`→fall through with `LOG_UNIMP`); the CD decoder skips QEMU's strict `CD_A=1` check because `smmu_init_ste_bypass`'s CD intentionally leaves `CD_A=0`. `S2_TRANS`/`ALL_TRANS`→build `SMMUTransCfg` from STE2/STE3 and dispatch QEMU's `smmu_ptw_64_s2`). `r100-rbdma` (OTO src+dst) and `r100-hdma` (LL walker LLP cursor + per-LLI SAR/DAR) translate through the hook via QOM `link<r100-smmu>` properties; SID 0 (PF) is the default for engine masters per Notion REBELQ SMMU Design § 1. **HDMA splits SIDs**: LL chain *cursor* uses SID 0 (stage-2 ALL_TRANS via the user PT); LLI *payload* SAR/DAR use `R100_HDMA_SMMU_SID_PAYLOAD = 8` (the FW's `smmu_init_ste_bypass(0)` STE → S1_TRANS + CD → `SMMU_BYPASS_PT`). `r100_smmu_enabled(s)` — public CR0.SMMUEN getter — exists so `r100-hdma`'s LLI dispatch can switch policy on the FW's "SMMU is live" signal. **Eventq + GERROR fault delivery**: every translate fault (`INV_STE`/`STE_FETCH`/walk) maps to a FW event_id (`r100_smmu_fault_to_event_id`, mirrors `q/sys/drivers/smmu/smmu.c:smmu_print_event` table — `INV_STE`→`C_BAD_STE=0x04`, `WALK`→`F_TRANSLATION=0x10`, `STE_FETCH`→`F_STE_FETCH=0x06`), packed as a 32 B SMMUv3 record (event_id + sid + input_addr + ipa, words 4-7 reserved), written to `EVENTQ_BASE_PA + (PROD&MASK)*32`, PROD advances with the wrap bit, GIC SPI 762 (`R100_INT_ID_SMMU_EVT`) pulses when `IRQ_CTRL.EVENTQ_IRQEN=1`. Eventq full → `r100_smmu_raise_gerror(EVTQ_ABT_ERR)` toggles `GERROR ^ GERRORN` (matches FW's `(GERROR ^ GERRORN) & ACTIVE_MASK` predicate), pulses GIC SPI 765 (`R100_INT_ID_SMMU_GERR`). Both IRQ outputs (`evt_irq` index 0, `gerr_irq` index 1) wired in `r100_create_smmu`. **Scope**: stage-2 + stage-1 (bypass STE), PF only (SID 0 + SID 8). **Debug surface**: optional `debug-chardev` property (chiplet 0 only — single-frontend `CharBackend`) wired via `-machine r100-soc,smmu-debug=<id>`; one ASCII line per translate / STE decode / PT-walk dispatch / CMDQ op / eventq emit / GERROR raise. CLI `--host` mode auto-creates `output/<name>/smmu.log`; `tests/scripts/{mem_dump,smmu_decode}.py` decode the in-DRAM STE / PT bytes the trace points to. See `docs/debugging.md` → "SMMU debug surface". **Deferred** (gated on workload need): per-SSID CD walk, multi-VF (SIDs 1..4 + 9..12), STE/IOTLB cache + honest invalidation, 2LVL stream tables, chiplet-0 PCIe-side TBU SID 17, dedicated HDMA-PA SID 16. See `docs/roadmap.md` → "SMMU v2" + `docs/rbdma-smmu-review.md`. |
...
| `r100_mailbox.{c,h}` | Samsung IPM SFR | 7 on chiplet 0 (VF0, PF; CM7_M4; plus the four DNC task queues) | Full register model (`INTGR/INTCR/INTMR/INTSR/INTMSR/ISSR0..63`) + two `qemu_irq` outs (INTMSR0/1). All ISSR writes funnel through `r100_mailbox_issr_store(src, ...)` with an `MbxIssrSrc` tag (`NPU_MMIO` / `CM7_STUB` / `HOST_RELAY`); NPU-sourced writes egress an `(bar4_off, val)` frame on `issr-chardev`, host-relayed writes deliberately skip the re-emit to avoid loopback. **CM7 mailbox stub** — chiplet-0 instance at `R100_CP0_MAILBOX_M4_BASE = 0x1FF1050000` realised with `cm7-stub=true` + `cm7-smmu-base=R100_SMMU_TCU_BASE` to terminate the PCIE_CM7 channel q-sys's `notify_dram_init_done` / `rbln_cm7_get_values` / `rbln_pcie_get_ats_enabled` use. ISSR reads always return `0` (matches the lazy-RAM-via-FREERTOS-VA shape every CM7 poll loop relies on — `ipm_samsung_write` lands at the cross-chiplet PA path, `ipm_samsung_receive` at the FREERTOS-VA path; both unmapped, so receive-side reads 0 and the loop exits with `val == 0`). Writes to `INTGR1` bit `cm7-dram-init-done-channel` (default 11 = `CM7_DRAM_INIT_DONE_CHANNEL`) with the just-stored `ISSR[<channel>] == 0xFB0D` synchronously poke `CR0.SMMUEN = 1` at `cm7-smmu-base + 0x20` via an `ldl_le_phys` / `stl_le_phys` RMW (preserves `CMDQEN` / `EVENTQEN`) — silicon-equivalent to `pcie_cp2cm7_callback → dram_init_done_cb → m7_smmu_enable`. Counter `cm7_dram_init_done_acks` (vmstate-tracked) records fires. This stub is the entire mechanism that lets chiplet 0's SMMU go from `CR0.SMMUEN=0` (identity bypass) to `CR0.SMMUEN=1` (real STE/PTW walks). Chiplets 1..3 don't need it — PCIE_CM7 lives on chiplet 0 only, so q-sys's `smmu_init` branches to `smmu_enable()` (plain MMIO write to `r100-smmu`) on worker chiplets. Public helpers exposed via `r100_mailbox.h`: `r100_mailbox_{raise_intgr,set_issr,get_issr,cm7_stub_write_issr,set_issr_words,fw_boot_done_seen}`. Everything else stays `static`. |
...
| `r100_hdma.{c,h}` | DesignWare dw_hdma_v0 reg block | 1 (chiplet 0) | MMIO at `R100_HDMA_BASE = 0x1D80380000` (32 × 0x800 channel slots, 16 WR + 16 RD). Per-channel state covers the full `struct hdma_ch_regs` surface from `q/cp/.../hdma_regs.h`. **Non-LL doorbell**: WR doorbell → `address_space_read` payload from chiplet sysmem at SAR + emit OP_WRITE; RD doorbell → emit OP_READ_REQ tagged `req_id = R100_HDMA_REQ_ID_CH_MASK_BASE | (dir<<5) | ch`, complete on matching OP_READ_RESP by `address_space_write` to chiplet sysmem at DAR. **LL doorbell** (`CTRL1.LLEN=1`): walks the chain at `LLP_LO|HI`, discriminating `dw_hdma_v0_lli` (24 B) from `dw_hdma_v0_llp` (16 B jump) on `control & LLP`; terminates on `LIE`. **Per-LLI dispatch (P10) gates on `r100_smmu_enabled(s->smmu)`**: SMMUEN=0 (p5 test, NPU-only smoke, very early boot before q-sys's `notify_dram_init_done`) → D2D in-process loop (`address_space_read` → `address_space_write`, 64 KB scratch chunks). SMMUEN=1 (production: q-cp drove `cmd_descr_hdma` post-CM7-handshake) → direction-based: WR ⇒ SAR walks SID 8 (FW bypass STE) + chunked OP_WRITE to host (DAR is the raw host PCIe IOVA the kmd published); RD ⇒ SAR is the raw host IOVA + chunked OP_READ_REQ + parked `qemu_cond_wait_bql()` on a per-channel `resp_cond` until OP_READ_RESP arrives, DAR walks SID 8 + `address_space_write`. The pre-P10 address-classifier (`addr ≷ REMU_HOST_PHYS_BASE` ⇒ host-bound) is gone; on real silicon the address itself selects PCIe RC vs NoC + SMMU-600, but REMU folds onto direction because q-cp's production path always crosses domains (WR = NPU→host, RD = host→NPU). LL chain *cursor* still walks SID 0 (stage-2 ALL_TRANS via the user PT) regardless of dispatch direction. Single GIC SPI 186 line + per-channel pending bit set in SUBCTRL_EDMA_INT_CA73 (`0x1FF8184368`, plain RAM in pcie-subctrl) on chain end. STOP-doorbell while idle is silently absorbed (matches DW silicon — gated on `c->enable & R100_HDMA_ENABLE_BIT`). Owns the `hdma` chardev (CharBackend is single-frontend); demuxes incoming OP_READ_RESP locally for the 0x80..0xBF channel-op partition. Public emit API in `r100_hdma.h` (`r100_hdma_emit_{write,write_tagged,read_req,read_resp}`). |
