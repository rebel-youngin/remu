...
| `r100_smmu.{c,h}` | SMMU-600 TCU | 1 / chiplet | **MMIO**: `CR0→CR0ACK` + `GBPA.UPDATE` auto-clear; `STRTAB_BASE`/`STRTAB_BASE_CFG` cache the in-DRAM stream-table geometry; `CMDQ_PROD` walks `(old_cons, new_prod]` and recognises `CMD_SYNC` (zeroes msiaddr per `CS=SIG_IRQ`) + the full `CMD_TLBI_*` / `CMD_CFGI_*` / `CMD_PREFETCH_*` opcode set (logged, advance CONS as no-ops — v1 has no STE / IOTLB cache to invalidate, every translate re-reads the STE). **Public translate API** (`r100_smmu_translate`, P11): NPU-side engines that consume DVAs call this before `address_space_*`. Pre-`CR0.SMMUEN`: identity (matches Arm-SMMU pre-enable bypass). Post-enable: read STE for `sid` from `STRTAB_BASE_PA + sid * 64`, decode `STE0.{V, config}` (`BYPASS`→identity, `ABORT`→`INV_STE` fault, `S1_TRANS`→v1 identity+LOG_UNIMP, `S2_TRANS`/`ALL_TRANS`→build `SMMUTransCfg` from STE2/STE3 and dispatch to QEMU's existing `smmu_ptw_64_s2`). `r100-rbdma` (OTO src+dst) and `r100-hdma` (LL walker LLP cursor + per-LLI SAR/DAR for D2D, NPU→host OP_WRITE chunks, host→NPU OP_READ_REQ chunks) translate through the hook via QOM `link<r100-smmu>` properties; SID 0 (PF) hardcoded per Notion REBELQ SMMU Design § 1. `r100-pcie-outbound` keeps its `host-ram` alias (P10-fix), `r100-dnc-cluster` cmd_descr fields stay untranslated until P6 surfaces a workload. **v1 scope** (P11): stage-2 only, PF only. **v2 deferred** (gated on workload need): stage-1 + CD walk, multi-VF (SIDs 1..4 with per-VF tables), STE/IOTLB cache + honest `CMD_TLBI_*` / `CMD_CFGI_*` invalidation, 2LVL stream tables, eventq/GERROR fault delivery, chiplet-0 PCIe-side TBU SID 17. Verified end-to-end via `./remucli test p11` (gdbstub-driven 3-level stage-2 walk + RBDMA OTO between IPAs `0x100000000` / `0x100001000`). |
...
| `r100_hdma.{c,h}` | DesignWare dw_hdma_v0 reg block | 1 (chiplet 0; M9-1c + P5 LL walker + P11 SMMU translate) | MMIO at `R100_HDMA_BASE = 0x1D80380000` (32 × 0x800 channel slots, 16 WR + 16 RD). Per-channel state for SAR/DAR/XferSize/doorbell/status/int_status. WR doorbell → `address_space_read` payload from chiplet sysmem at SAR + emit OP_WRITE; RD doorbell → emit OP_READ_REQ tagged `req_id = R100_HDMA_REQ_ID_CH_MASK_BASE \| (dir<<5) \| ch`, complete on matching OP_READ_RESP by `address_space_write` to chiplet sysmem at DAR. P11 added the SMMU translate hook in `r100_hdma_lli_{wr,rd,d2d}` and the LLP walker — NPU-side SAR/DAR/cursor go through `r100_smmu_translate(SID=0, …)` (Notion REBELQ SMMU Design § 1: HDMA-IPA on PF) before each `address_space_*` call, with an identity fallback when `CR0.SMMUEN=0` or the engine has no smmu link. Single-line GIC SPI 186 + per-channel pending bit set in SUBCTRL_EDMA_INT_CA73 (`0x1FF8184368`, plain RAM in pcie-subctrl). Owns the `hdma` chardev (CharBackend is single-frontend); demuxes incoming OP_READ_RESP locally for the 0x80..0xBF channel-op partition. Other partitions are reserved: 0x00..0x7F (legacy cm7 BD-done at 0x01..0x0F retired in P7; P1b cfg-mirror reverse-emit at 0x00 retired with the P10-fix shm cfg-shadow alias; available for UMQ multi-queue), 0xC0..0xFF (formerly r100-pcie-outbound synchronous PF-window reads; the device now aliases shared host-ram instead). Unhandled req_ids log `GUEST_ERROR`. Public emit API in `r100_hdma.h` (`r100_hdma_emit_{write,write_tagged,read_req,read_resp}`); the prior `r100_hdma_emit_cfg_write` and `r100_hdma_set_outbound_callback` helpers were retired with their respective shm-aliased paths. |
