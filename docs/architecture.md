...
| `r100_smmu.c` | SMMU-600 TCU | 1 / chiplet | `CR0→CR0ACK` + `GBPA.UPDATE` auto-clear; CMDQ walker zeroes `CMD_SYNC.MSI_ADDRESS` and auto-advances `CMDQ_CONS=PROD`. **Register-only — STE / CD / page tables FW writes to DRAM are never walked.** Every DVA-consuming engine (`r100-rbdma` SAR/DAR, `r100-hdma` SAR/DAR / LLP, `r100-dnc-cluster` cmd_descr fields, `r100-pcie-outbound` PF window) calls `address_space_{read,write}` directly with the FW-supplied address + a chiplet-base add, so REMU's effective transform is `S1 ∘ S2 = identity`. **Becomes a hard blocker** the moment a workload uses non-identity IOVAs — first hit by P10's umd `command_submission` cb[1] (LL chain with `llp` / `dar` in the SMMU stage-2 SYSTEM / CHIPLET0_USER IPA windows). Concrete plan-of-attack (with observed IPA addresses, FW page-table source files, and a comment on why the obvious `memory_region_init_alias` shortcut is the wrong fix) is in `docs/roadmap.md` § Long-term follow-ons → "SMMU honour FW page tables"; diagnostic post-mortem is in `docs/debugging.md` → "P10 cb[1] non-completion". |
