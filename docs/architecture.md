...
3. Overlays a 64 KB `SYSREG_CP0` window at the config-space address `R100_CP0_SYSREG_BASE` (`0x1FF1010000`), aliasing back to the chiplet's own private-alias `SYSREG_CP0` RAM at `chiplet_base + R100_SYSREG_CP0_PRIVATE_BASE` (`0x1E01010000`). BL1's cold-boot release path writes RVBAR via the QSPI bridge to the private-alias address, while BL31's `rebel_h_pm.c:set_rvbar()` (invoked from the PSCI `CPU_ON` SMC handler) writes RVBAR directly via this config-space address. The overlay makes both paths converge on the same backing RAM — matching silicon, where each chiplet's decoder routes `0x1FF1010000` chiplet-locally.
