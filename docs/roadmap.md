...
- [ ] **q-cp task-init triage (`hils.log`).** Even on chiplet 0 the CP1 task init isn't fully clean. Three distinct symptoms, likely three distinct device-model gaps:
  - [ ] `RDSN init timeout` — some task polls an RDSN-side register/ring that no REMU device model currently services. Trace the poll address from q-cp sources under `external/ssw-bundle/products/rebel/q/cp`, decide whether it lives inside an existing peripheral (RBC? RBDMA?) or needs a new tiny stub.
  - [ ] Zero-valued `RBDMA HW INFO` / `SHM HW INFO` dumps — FW reads ID / version / capability fields from RBDMA + SHM that we haven't populated. Seed plausible constants in the corresponding device models (or add minimal stubs if they don't exist yet).
  - [ ] Per-V/H RBC `TimeoutWindow` prints — may be benign (FW just logging a programmed value), or may indicate RBC link state the FW doesn't like. Confirm by reading the FW source the prints come from; if it's purely informational, leave it and document so; otherwise fix in `r100_rbc.c`.
