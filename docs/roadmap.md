...
- `external/qemu` is a git submodule pinned at upstream tag `v9.2.0` (no source patches). `./remucli build` installs symlinks (`hw/arm/r100/`, `include/hw/arm/{r100_soc,remu_addrmap}.h`) and injects one `subdir('r100')` line into `hw/arm/meson.build` at first build. All R100 device models live in `src/machine/` and are wired in via the symlinked subdir; `.gitmodules` sets `ignore = dirty` so those build-time mods stay off the superproject's `git status`. Fresh checkouts: `git submodule update --init external/qemu`, then `./remucli build`.
