...
- [ ] **Secondary-chiplet CP1 bring-up verification.** Chiplet 0's CP1 cluster is observed through the HILS ring tail (`cp_create_tasks_impl`, RBDMA / SHM HW INFO dumps, per-V/H RBC `TimeoutWindow` prints). Chiplets 1-3's CP1 halves are released by BL2 and `arm_set_cpu_on` fires for each, but there's no per-chiplet HILS ring tail wired up, so we haven't directly confirmed those 12 vCPUs (3 chiplets × CP1.cpu{0..3}) reach FreeRTOS task init. Plan:
  1. Attach GDB and `info threads` / per-thread `frame 0` to confirm CP1.cpu0 on chiplets 1-3 is running FreeRTOS code (not spinning in BL31 PSCI wait, not faulted).
  2. If gaps show up, either wire the HILS ring tail to each chiplet's own `.logbuf` region, or (cheaper) add a second chardev output for chiplet-N CP1 task logs.
