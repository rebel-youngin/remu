...
| P10 SMMU bypass STE | Honest stage-1 walking for the FW's bypass STE (`smmu_init_ste_bypass(0)` programs `STE 8 = S1_TRANS + CD → SMMU_BYPASS_PT`). `r100_smmu` decodes STE1.S1DSS, fetches the CD from `STE0.S1ContextPtr`, validates per Arm SMMUv3 § 5.3 (skipping the `CD_A=1` strict check the FW's bypass-CD intentionally leaves unset), populates `SMMUTransCfg.tt[0..1]` from `R100_CD_TSZ/TG/EPD/IPS/AFFD/TBI/ASID/R/TTB`, and dispatches `smmu_ptw_64_s1` against the FW's bypass PT (`smmu_create_bypass_table` builds it from `bypass_regions[]`, HTID0 identity-maps `0x40000000..0x8000000000` for the local chiplet, HTID1..15 redirect remote chiplets). New constants: `R100_HDMA_SMMU_SID_LL_PTR = 0` (LL chain cursor → stage-2 ALL_TRANS via user PT) + `R100_HDMA_SMMU_SID_PAYLOAD = 8` (LLI SAR/DAR → bypass STE 8). `r100_hdma_translate(sid, …)` dispatches per call site. The v1 "S1_TRANS collapses to identity" shortcut at the old `r100_smmu.c:1089` is gone; `S1DSS=BYPASS` still gives identity (silicon-correct, since FW's `smmu_init_ste` sets it for the regular SIDs 0..4 to disable stage-1 pre-`smmu_s2_enable`); `S1DSS=SUBSTREAM0` always walks CD[0] (substream-keyed lookup is v2). Faults route through `r100_smmu_emit_event` so FW's `smmu_event_intr` reads a real eventq entry. m5..p5 + p11 + p11b regression green; `./remucli test p10` advances past the prior "stage-2 fault on raw LLI SAR" wedge into the upstream-of-SMMU HDMA dispatch issue (raw IOVA → host-leg routing, see P10 row in the open-milestones table below). | (this commit) |
...
| P10 | umd smoke test in `guest/` — `command_submission -y 5` → 2 CBs submitted → 2 MSI-X completions → exit 0. **Three layers landed**: (1) **CM7 mailbox stub** (chiplet-0 `MAILBOX_CP0_M4` instantiated as a real `r100-mailbox` with `cm7-stub=true` + `cm7-smmu-base=R100_SMMU_TCU_BASE`) — on q-sys's `notify_dram_init_done` raising `INTGR1` bit 11 with `ISSR[11] == 0xFB0D`, the stub pokes `CR0.SMMUEN=1` synchronously inside the MMIO write handler — silicon-equivalent to `pcie_cp2cm7_callback → dram_init_done_cb → m7_smmu_enable`. ISSR reads always return 0 to preserve the lazy-RAM-via-FREERTOS-VA "first read returns 0" shape every other CM7 poll loop in q-sys depends on (`rbln_cm7_get_values` / `rbln_pcie_get_ats_enabled`) for early exit. (2) **HDMA SAR/DAR via FW bypass STE 8 + honest stage-1 walk**. `r100_hdma_translate` now takes a `sid` parameter; the LL chain *cursor* uses `R100_HDMA_SMMU_SID_LL_PTR=0` (stage-2 ALL_TRANS via the user PT) and the LLI *payload* SAR/DAR use `R100_HDMA_SMMU_SID_PAYLOAD=8` (the FW's `smmu_init_ste_bypass(0)` STE — `S1_TRANS` + CD pointing at `SMMU_BYPASS_PT`). On the SMMU side, `r100_smmu`'s S1_TRANS path no longer collapses to a "v1 identity" shortcut — the walker reads STE1.S1DSS, fetches the CD from `STE0.S1ContextPtr`, decodes the FW's `smmu_init_cd_bypass` programming (T0SZ=20, TG0=4 KB, IPS=40, EPD1=1, AFFD=1, R=1, AA64=1) and dispatches QEMU's `smmu_ptw_64_s1` against `SMMU_BYPASS_PT` (FW's `smmu_create_bypass_table` builds it from `bypass_regions[]`, HTID0 identity-maps `0x40000000..0x8000000000` for the local chiplet, HTID1..15 map the same window to remote chiplets at `c*0x10000000000 + 0x40000000`). CD-validation drops QEMU's strict `CD_A=1` check because `smmu_init_cd_bypass` deliberately leaves CD_A unset (real SMMU-600 doesn't fault that bypass CD; rejecting it would defeat the "make the FW SMMU init's impact real" point). (3) **HDMA LLI direction-based dispatch (this commit)**. `r100_hdma_walk_ll` no longer classifies LLI fields by `addr ≷ REMU_HOST_PHYS_BASE`; instead it gates on `r100_smmu_enabled(s->smmu)` (a new public CR0.SMMUEN getter on `r100-smmu`). Pre-enable (p5 test, NPU-only smoke, very early boot — `output/p5-hdma/smmu.log` confirms p5's gdbstub kick fires while `cr0_smmuen=0`): D2D — both LLI endpoints address chiplet-local DRAM and the walker runs `address_space_read` → `address_space_write`. Post-enable (production: q-cp drove `cmd_descr_hdma` after q-sys's CM7 handshake): direction-based — `dir == WR` ⇒ SAR walks SID 8, DAR is the raw host PCIe IOVA the kmd published and goes out the host-leg `OP_WRITE` chardev; `dir == RD` ⇒ SAR is the raw host IOVA (`OP_READ_REQ` + parked `qemu_cond_wait_bql()` for `OP_READ_RESP`), DAR walks SID 8. The "stage-2 fault on raw LLI SAR" wedge that ate cb[0] is **gone** — verified by `output/p10-umd/hdma.log` showing 515 `OP_READ_REQ`/`OP_READ_RESP` round-trips for cb[0]'s 3-LLI chain (2 MB + 8 KB + 4 KB), `ll_walk_end ... last_seen=1`, `signal_completion completions=1`, and `output/p10-umd/msix.log` carrying one MSI-X frame at vector 0 (q-cp's `cb_complete → pcie_msix_trigger`). cb[0] now completes end-to-end through the silicon-accurate path. **Remaining**: the kmd hangs in `rebel_hw_init+0x439` post-cb[0] / pre-cb[1] — the upstream `readl_poll_timeout_atomic(jiffies_to_usecs(...))` unit-mismatch documented in `docs/debugging.md` (3 s timer becoming a ~3-hour busy-loop on TCG; stock-KMD bug, not REMU's). cb[1] / RBDMA dispatch may also surface follow-on issues but is unobservable until the timeout cascade clears. m5..p5 + p11 + p11b regression green; `./remucli test p10` still hits the 180 s budget but the failure mode has moved past every HDMA / SMMU / cfg-mirror issue. See `docs/debugging.md` → "P10 — HDMA payload SAR/DAR via FW SMMU bypass". | mostly landed (next-edge is upstream-of-REMU) | excluded from `./remucli test` defaults via `in_default=False` |
...
1. **P10** — cb[0] end-to-end now lands cleanly (CM7 stub +
   ...
   `rebel_hw_init+0x439` (see `docs/debugging.md` →
