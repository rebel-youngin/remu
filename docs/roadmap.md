...
**All four chiplets now reach `REBELLIONS$` on CP0, and all 16 CP1 vCPUs run q-cp FreeRTOS steady-state code** — `Hello world FreeRTOS_CP` / `hw_init: done` / `REBELLIONS$` on each chiplet's own UART (`uart{0,1,2,3}.log`), the HILS ring tail drains chiplet 0's CP1 task init end-to-end (`chiplet count is 4`, `cp_create_tasks_impl +/-`, DNC[0..15] cdump init, RBDMA / SHM HW INFO dumps, ICPI init, CS tasks 1/2/3 started on CP1.cpu{1,2,3}), and GDB inspection across all 32 threads shows every chiplet's CP1.cpu{0..3} sitting in `ipm_samsung_receive` / `taskmgr_fetch_dnc_task_worker_cp1` steady-state (see `tests/scripts/gdb_inspect_cp1.gdb`). Two fixes unblocked the chiplet-0 path originally:
...
With those in, **BL31 now completes `plat_arm_gic_driver_init()` + `gicv3_rdistif_probe()` on every CP0.cpu0 + CP1.cpu0, and all four chiplets reach the `Hello world FreeRTOS_CP` / `hw_init: done` / `REBELLIONS$` shell on their own UART.** The HILS ring drains chiplet 0's CP1 FreeRTOS task init end-to-end: `chiplet id 0`, `cp_create_tasks_impl +`, DNC[0..15] cdump init, SGLUE init, `RBDMA HW INFO: rel_date 0x20260101 rbdma ver 1 MRV ver 1.0 chiplet_id 0`, `SHM0 HW INFO: rel_date 0x20260101 ip ver 1.0.0`, ICPI init, substream manager, command-stream tasks 1/2/3 initialized then started on CP1.cpu{1,2,3}, abort task, bootdone task, and `[rst] chiplet reset coordinator task started` — all cleanly. `info mtree -f` confirms each chiplet view resolves `gicv3_dist` / `gicv3_redist_region[0]` at priority 10 (aliases into chiplet-absolute sysmem) and `info qtree` shows four `arm-gicv3` devices with `first-cpu-index` 0, 8, 16, 24.

Getting CP1 task init clean on chiplet 0 required adding stubs for three register windows that CP0-side BL1/BL2/BL31/FreeRTOS never touch but CP1's q-cp does (see `src/machine/r100_dnc.c`):

- **RDSN MGLUE** at `DCL{0,1}_CFG_BASE + 0x20000` — `rdsn_init()` polls `RDSN_HEAD_STATUS0 & 0x000F000F == 0x000F000F` (per-row `init{0..3}` + `config{0..3}` bits); `rdsn_sanity_check()` fires dtest0/dtest3 and waits for `TE0_RPT0.valid+pass` / `TE3_RPT0.valid+pass` (bits 1 and 2 set).
- **SHM banks** at `DCL{0,1}_CFG_BASE + 0x10000` (16 × 1 KB) — `shm_init()` triggers TPG per bank and polls `INTR_VEC.tpg_done` (bit 0). Default reads return `tpg_done=1` with error bits clear.
- **DNC slots** at `DCL{0,1}_CFG_BASE + 0x00000` (8 × 8 KB) — `dnc_register_ops()` checks `DNC_CONFIG_IP_INFO1.ip_ver` against `DNC_V1_0`/`DNC_V1_1` (accepts 1 or 2, aborts with `ERR_DNC` otherwise); `dnc_run_sp_test()` polls `DNC_STATUS_SP_STATUS01.test_done` after writing `CONFIG.SP_TEST.trig=1` and times out after 1 ms without it. Stub advertises `ip_ver = DNC_V1_1` and permanent `test_done = 1`.
- **RBDMA CFG window** at `NBUS_L_RBDMA_CFG_BASE = 0x1FF3700000` — `rbdma_get_ip_info()` reads `IP_INFO0..5` and logs them; cosmetic-only, seeded with plausible rel_date / ip_ver / chiplet_id so the dump reads non-zero.

All three windows share a sparse-writeback (`GHashTable`) register file so FW write-then-read round-trips work; only the polled status / version fields have synthetic defaults. See `r100_dnc.c` for the override table and `r100_soc.c:r100_chiplet_init` for the two DCL + one RBDMA instances wired per chiplet at priority 1 on `cfg_mr` (same pattern as SYSREG_CP* and RBC).

Residual `hils.log` items on chiplet 0 are now all cosmetic / informational: `failed to restore pcie event log` (postproc CM7 fallback, INFO), per-V/H RBC `TimeoutWindow req=50000000 ns prog=50069504 ns` (RBCM_LOGI after writing the config register, purely informational), `DCL{0,1} ip_info: 0 0 0 0` (SGLUE IP info at `DCL_CFG + 0x70000` — we haven't seeded SGLUE yet and the FW just logs it), and `RBLN-CPIF: SPI transmit failed (res: -16)` (QSPI post-boot activity, doesn't gate task init). None call `abort_event` or prevent CP1 tasks from running.
...
| `r100_dnc.c` | DCL CFG window + RBDMA | Two `r100-dnc-cluster` instances per chiplet covering the 1 MB DCL0 / DCL1 cfg windows (`0x1FF2000000` / `0x1FF2800000`): DNC slots at offset `0x00000` (8 × 8 KB) advertise `DNC_CONFIG_IP_INFO1.ip_ver = DNC_V1_1` and `DNC_STATUS_SP_STATUS01.test_done = 1` so `dnc_register_ops` / `dnc_run_sp_test` pass on iteration 0; SHM banks at `0x10000` (16 × 1 KB) return `INTR_VEC.tpg_done = 1` and seed `IP_INFO0..3` (rel_date, ver, latency, n_bank=16, capa≈256 MB) so `shm_init()` TPG poll and `shm_get_ip_info()` print cleanly; MGLUE at `0x20000` returns `RDSN_HEAD_STATUS0 = 0x000F000F` (ALL_PREPARED) plus `TE{0,3}_RPT0 = 0x6` (valid+pass) for `rdsn_init` / `rdsn_sanity_check`. Plus one `r100-rbdma` per chiplet at `NBUS_L_RBDMA_CFG_BASE = 0x1FF3700000` (1 MB) seeding `IP_INFO0..2` (rel_date, ver, chiplet_id). Sparse `GHashTable` write-back (same pattern as `r100_hbm.c`) so RMW round-trips work. |
...
- [x] **Secondary-chiplet CP1 bring-up verification.** Confirmed via GDB (`aarch64-none-elf-gdb -ex 'target remote :1234' -x tests/scripts/gdb_inspect_cp1.gdb`) after a 60 s wall-clock boot: all 16 CP1 vCPUs (4 chiplets × 4 cores) are running q-cp FreeRTOS steady-state code. Per-chiplet frame-0 layout matches chiplet 0 exactly:
  - `CP1.cpu0` → `drivers/mailbox/mailbox.c:ipm_samsung_receive` (ICPI inbox poll).
  - `CP1.cpu1..3` → `task_mgr/cp1/fetch_task.c:taskmgr_fetch_dnc_task_worker_cp1` (DNC fetch workers for DNCs 0-5 / 6-10 / 11-15).

  PCs land in the local `R100_CP1_FREERTOS_BASE` view (`0x14200000 + off`) with `ELR_EL3` resolving to the correct chiplet-absolute DRAM image (chiplet 0 → `0x14200000`, chiplet 1 → `0x2014200000`, chiplet 2 → `0x4014200000`, chiplet 3 → `0x6014200000`) — i.e. each chiplet is executing against its own `FW_PER_CHIPLET_IMAGES` copy at the right base. No `plat_panic_handler` / PSCI-wait spins. See `tests/scripts/gdb_inspect_cp1.gdb` (pure-GDB script — the bundled `aarch64-none-elf-gdb` has no Python) and the tail-end of the `Testing & Debugging` doc for the exact invocation.
- [x] **q-cp task-init triage (`hils.log`).** Chiplet 0's CP1 task init is now clean; all three symptoms resolved (`src/machine/r100_dnc.c`, `r100_soc.c:r100_chiplet_init`):
  - [x] `RDSN init timeout` — `rdsn_is_prepared()` polls `RDSN_HEAD_STATUS0 & 0x000F000F == 0x000F000F` at DCL CFG + 0x20010, `rdsn_sanity_check()` waits for `TE{0,3}_RPT0` valid+pass. `r100-dnc-cluster`'s MGLUE defaults now return the satisfied pattern.
  - [x] Zero-valued `RBDMA HW INFO` / `SHM HW INFO` dumps — `rbdma_get_ip_info()` and `shm_get_ip_info()` read `IP_INFO0..3/5` from the DCL / RBDMA CFG windows. Seeded plausible rel_date / ver / chiplet_id / n_bank / capacity constants in `r100_dnc.c`; the log now prints `RBDMA HW INFO: rel_date: 0x20260101 rbdma ver: 1 MRV ver: 1.0 chiplet_id: 0` and `SHM0 HW INFO: rel_date: 0x20260101 ip ver: 1.0.0 ...`. (A related DNC IP-version check — `dnc_register_ops()` expecting `ip_ver ∈ {DNC_V1_0, DNC_V1_1}` — and `dnc_run_sp_test()`'s `SP_STATUS01.test_done` poll emerged once RDSN/SHM unblocked; both covered by the same stub.)
  - [x] Per-V/H RBC `TimeoutWindow` prints — confirmed benign. Source is `external/ssw-bundle/products/rebel/q/sys/drivers/rbcm/rbcm_core.c` `RBCM_LOGI("[%s] TimeoutWindow req=%u ns prog=%llu ns ...")` which fires on every successful `rbcm_reg_writel(Timeout_window_config0)`. Pure informational, no follow-on check.

  Residual cosmetic items we left alone: `DCL{0,1} ip_info: 0 0 0 0` (SGLUE at DCL CFG + 0x70000 — FW prints only, no follow-on check, so no stub added), `failed to restore pcie event log` (postproc event-log magic miss → fallback), `RBLN-CPIF: SPI transmit failed (res: -16)` (post-boot QSPI activity).
- [ ] **Fix `sys/build.sh` `debug` subcommand.** It currently resets CLI flags, so `CHIPLET_COUNT=1` builds (useful for single-chiplet, smaller-trace iteration) don't work. Small DX fix upstream in `external/ssw-bundle/.../q/sys/build.sh`; not blocking Phase 2 but worth doing while we're still in Phase 1 land.

HBM3 PHY training fidelity is **out of scope** — the current stub (compile-time "done" sentinels + 0 for everything else) drives the full `hbm3_phy_training_evt1` sequence to completion on every channel, emitting only the benign `read optimal vref (vert) FAILED. No valid window found.` line which the FW handles via its documented fallback path. A more faithful model would silence that line but buys nothing for later phases.

### Success criteria

- All 4 chiplets print FreeRTOS startup banner on their UART (both CP0 and CP1 `Hello world FreeRTOS_CP`) ✓ (CP0 visible on each chiplet's UART; CP1 confirmed via HILS ring on chiplet 0 and via GDB thread dump on chiplets 1-3)
- Chiplet 0 discovers 3 secondary chiplets via QSPI bridge ✓
- All 32 vCPUs (4 chiplets × 2 clusters × 4 cores) reach the FreeRTOS shell or its equivalent on their respective cluster ✓ (CP0 at `REBELLIONS$` on all 4 chiplets; CP1.cpu0 in `ipm_samsung_receive`, CP1.cpu{1,2,3} in `taskmgr_fetch_dnc_task_worker_cp1` on all 4 chiplets per `tests/scripts/gdb_inspect_cp1.gdb`)
- GDB can step through FW code on any chiplet's CP0 or CP1 vCPU ✓ (see `docs/debugging.md` — GDB workflow)
