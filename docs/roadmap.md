...
## Phase 2: Host Drivers — in progress (M1-M8a done, M8b Stage 1 + Stage 2 plumbing done)
...
| M8b | `FW_BOOT_DONE` + `TEST_IB` ring | in progress (Stages 1, 2, 2c done; Stage 3 starting) | First real protocol driven over the M8a ISSR plumbing: kmd polls BAR4+MAILBOX_BASE+0x10 for `FW_BOOT_DONE` (0xFB0D in ISSR[4]), then kmd and q-cp HIL exchange `TEST_IB` messages on the shared-DRAM ring. Approach (a): real kmd against full q-sys FW end-to-end. Staged in three sub-milestones (see next table) |
...
| 3 — `TEST_IB` ring | Stage 2c cleared — Phase 2 now has a working kmd-driven doorbell all the way to `r100-mailbox`. Remaining work: with `--host`, FreeRTOS starts the scheduler but `bootdone_task` no longer emits `FW_BOOT_DONE` on ISSR[4] (Phase 1 alone does). The 4 additional kmd frames unlocked by Stage 2c (`0x98 val=0x3`, `0x1c val=0x40` group-1 trigger) land during FreeRTOS startup and steer the CP FW into a different path (soft-reset accepted? TEST_IB-pre-boot handshake?). Next step: add a light-weight HILS-visible trace in `bootdone_service.c` / the PF-mailbox INTGR handler so we can see what the kmd's soft-reset sequence actually looks like from the FW side, then drive the `kmd` ↔ `q-cp HIL` `TEST_IB` exchange on the shared-DRAM ring. | in progress |
