...
- **`./remucli run --host`**: dual-QEMU orchestrator. Auto-verifies both bridges end-to-end: M4/M5 via PCI enumeration + host/NPU `info mtree` + `/proc/<pid>/maps`; M6 via host `info mtree` (`r100.bar4.mmio` overlay) + NPU `info qtree` (both `r100-doorbell` and `r100-mailbox` present). Both QEMU monitors exposed as unix sockets under `output/<name>/{host,npu}/monitor.sock`.

### Pending components

- **MSI-X routing**: eventfd-based reverse-direction interrupt forwarding (M7). The NPU-side `r100-mailbox` already exposes silicon-accurate GIC SPIs (184/185) on chiplet 0 — M7 just needs the reverse mapping so a FW write to `MAILBOX_INTGR{0,1}` on the NPU reaches the x86 guest via `msix_notify()` on `r100-npu-pci`.
