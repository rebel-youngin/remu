...
4. **Per-chiplet GIC MMIO aliases** (`r100_build_chiplet_view` in `src/machine/r100_soc.c`) — on silicon each chiplet has its own GIC600 with distributor at `R100_GIC_DIST_BASE = 0x1FF3800000` and two 4-frame redistributor regions at `R100_GIC_REDIST_CP{0,1}_BASE = 0x1FF3840000 / 0x1FF38C0000`, all hardcoded in the FW per-chiplet. REMU keeps **a single shared `arm-gicv3`** (`num-cpu=32`, one contiguous 32-frame redistributor region) mounted once in sysmem, and layers per-chiplet priority-10 aliases in each chiplet's CPU view:
   - **Distributor alias** at `R100_GIC_DIST_BASE` (0x10000 window) pointing at the shared `gicv3_dist` MMIO — supersedes the chiplet-wide `cfg_mr` unimpl catch-all, which had been absorbing the `GICD_PIDR2` read at `0x1FF380FFE8` on secondaries (shadowed value 0 tripped `gicv3_driver_init()`'s `ARCH_REV_GICV3` assertion → `plat_panic_handler`).
   - **CP0 / CP1 redistributor aliases** at `R100_GIC_REDIST_CP{0,1}_BASE` (4 frames each, 0x80000), each pointing at that chiplet's own 4-frame slice of the shared 32-frame layout — so `gicv3_rdistif_probe()` on chiplet N matches on the first frame (its own CP0.cpu0 / CP1.cpu0) instead of walking into chiplet 0's frames and mismatching MPIDR.

   This avoids patching `external/qemu`. Trade-off: SPIs share a single distributor, so per-chiplet UART SPI 33 routing is shared (only chiplet 0's UART wires to the GIC); fine for Phase 1 (TF-A is polling-mode), to revisit in Phase 2+ when secondary FreeRTOS runs interrupt-driven UART.

With those in, BL31 now prints the `BL31: Chiplet-N / Boot reason: Cold reset(POR)` banner on all four chiplets. `info mtree -f` confirms each chiplet view has its own `gicv3_dist` at `0x1FF3800000` and two cluster-local `gicv3_redist_region` windows resolving to the correct slice of the shared device. **Expected next milestone: secondary CP0.cpu0 passes `gicv3_driver_init()` end-to-end**, landing at `rebel_bakery_lock_acquire` — the known downstream spin where CP0 waits on its CP1 peer. Pending a test run on `-p silicon` to confirm via GDB and to identify the next CP1.cpu0 boundary.
...
|---|---|---|
| `r100_soc.c` | Machine type | 4 chiplets × 8 `cortex-a72` CPUs (MIDR overridden to CA73 r1p1 `0x411FD091`, MPIDR `chiplet << 24 \| cluster << 8 \| core` — FW masks bits 24-31 out when computing `plat_core_pos_by_mpidr`); single shared `arm-gicv3` (`num-cpu=32`, one 32-frame redistributor region) mounted in sysmem at `R100_GIC_DIST_BASE` / `R100_GIC_REDIST_CP0_BASE`, plus per-chiplet priority-10 aliases in each chiplet's CPU view that retarget the distributor and the two 4-frame cluster-local redistributor windows (`R100_GIC_REDIST_CP{0,1}_BASE`) to that chiplet's own 8 frames — so `GICD_PIDR2` and `gicv3_rdistif_probe()` resolve locally on every chiplet; per-chiplet `MemoryRegion` view overlays the 256 MB `PRIVATE_BASE` (`0x1E00000000`) window onto each chiplet's own slice of `sysmem`, and the 64 KB `SYSREG_CP0` / `SYSREG_CP1` blocks at config-space `0x1FF1010000` / `0x1FF1810000` onto the private-alias copies; `MSR S1_1_C15_C14_0, x0` stubbed as `ARM_CP_NOP` (BL31 EL3 MMU enable); per-chiplet 16550 UART on dedicated chardevs (chiplet 0's UART wires to shared-GIC SPI 33; secondaries are output-only for Phase 1); shared flash RAM at `0x1F80000000` (64 MB, aliased per chiplet). |
...
- `external/qemu` is a git submodule pinned at upstream tag `v9.2.0` (no source patches). `scripts/build-qemu.sh` installs symlinks (`hw/arm/r100/`, `include/hw/arm/{r100_soc,remu_addrmap}.h`) and injects one `subdir('r100')` line into `hw/arm/meson.build` at first build. All R100 device models live in `src/machine/` and are wired in via the symlinked subdir; `.gitmodules` sets `ignore = dirty` so those build-time mods stay off the superproject's `git status`. Fresh checkouts: `git submodule update --init external/qemu`, then `remu build` (or `./scripts/build-qemu.sh`).
...
Chiplets 1-3 (`/tmp/remu_uart{1,2,3}.log`) run the same sequence in parallel against their own `r100_hbm.c` instance up through `Release a reset of CP1.cpu0`, then BL31 on each chiplet prints its own banner (`NOTICE:  BL31: Chiplet-N / Boot reason: Cold reset(POR)` / `v2.9.0(debug)`). With the new per-chiplet GIC distributor + redistributor aliases, `gicv3_driver_init()` is expected to complete on every chiplet's CP0.cpu0 — see "BL31 bring-up on secondary chiplets" below for the pending verification. Chiplet 0's CP1 cluster boots `freertos_cp1.bin` without its own UART; the HILS ring tail (`/tmp/remu_hils.log`) drains its task init output (`chiplet count is 4`, `cp_create_tasks_impl`, RBDMA / SHM HW INFO, per-V/H RBC `TimeoutWindow` prints). The `ERROR: CH[N] read optimal vref (vert) FAILED. No valid window found.` lines on each chiplet are the expected FW fallback path (`phy_read_recv_vref_training` returns without a valid window, FW retries at `0xfffe`, PRBS training reports Done, `HBM Training Complete` still fires) — not a failure.
...
- [~] **BL31 bring-up on secondary chiplets** (partial — banner reached on all 4; `gicv3_driver_init()` completion pending verification): all four chiplets now ERET from BL2 into their own BL31 image and print the BL31 banner on their UARTs. Three blockers on the `gicv3_driver_init()` path:
  1. `FAR_EL3 = 0x8001ec08b0` data abort from an MPIDR encoding mismatch (FW's `plat_core_pos_by_mpidr` requires `Aff2 == Aff3 == 0`; the chiplet id is now in MPIDR bits 24-25 which the FW masks out). **Fixed**.
  2. `gicv3_rdistif_probe()` on secondaries previously read chiplet 0's `GICR_TYPER` (MPIDR mismatch → `el3_panic`) because the single 32-frame redistributor laid frames out in `cpu_index` order. **Fixed** by per-chiplet redistributor aliases in `r100_build_chiplet_view` (each chiplet's view remaps `R100_GIC_REDIST_CP{0,1}_BASE` to its own 4+4-frame slice).
  3. `GICD_PIDR2 = 0` shadow on secondaries — the chiplet-wide `cfg_mr` unimpl catch-all (installed at `chiplet_base + R100_CFG_BASE`) absorbed the distributor read at offset `0x380_FFE8` before it could fall through to sysmem's real `gicv3_dist`, so `gicv3_driver_init()` tripped the `ARCH_REV_GICV3` assertion and landed in `plat_panic_handler`. **Fixed** by adding a priority-10 distributor alias in each chiplet's CPU view at `R100_GIC_DIST_BASE`, pointing at the shared `gicv3_dist` MMIO. `info mtree -f` confirms each chiplet view resolves `gicv3_dist` at `0x1FF3800000` and the two cluster-local redistributor windows at `0x1FF3840000` / `0x1FF38C0000`.

  **Expected next milestone** (pending test run): secondaries' CP0.cpu0 pass `gicv3_driver_init()` and land at `rebel_bakery_lock_acquire` — the known downstream spin where CP0 waits on its CP1 peer. Chiplet 0 remains fully operational: `REBELLIONS$` on UART0, CP1 FreeRTOS tasks drained via HILS ring. Next: run on `-p silicon`, use GDB (`info threads`, per-thread `frame 0`) to confirm cpus {8,16,24} reach `rebel_bakery_lock_acquire`, and identify the CP1.cpu0 boundary on secondaries.
...
| GIC600 (x4) | Single shared `arm-gicv3` (num-cpu=32, one 32-frame redistributor region) with per-chiplet distributor + cluster-local redistributor aliases in each chiplet's CPU view at priority 10, so the FW's chiplet-local `R100_GIC_DIST_BASE` / `R100_GIC_REDIST_CP{0,1}_BASE` addresses resolve to that chiplet's slice of the shared device | Per-chiplet GIC (four `arm-gicv3` instances) if secondary interrupt routing becomes necessary | Same |
| UART (x4) | Per-chiplet 16550 on dedicated chardevs + HILS ring tail (chiplet 0's UART wires to shared-GIC SPI 33; secondaries are output-only) | SPI 33 per-chiplet once per-chiplet GIC lands | Same |
