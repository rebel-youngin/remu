...
- [ ] FreeRTOS UART routing: get `q-sys` logs onto the serial console. Two approaches: (a) tail `[0x10000000, +logbuf_size)` from a REMU-side helper and print changes to stderr, or (b) model the DW-APB UART register set at `PERI0_UART0_BASE` (`0x1FF9040000`) so `uart_putc` reaches the QEMU `serial-mm` backend. Option (b) matches silicon behaviour better and also unblocks interrupt-driven uart_rx path
