...
2. **P2** — Delete the now-dead `r100_imsix_notify` call from
   `r100-cm7`'s Stage 3c BD-done state machine. After P1c the call
   is unreachable on a stock run, but it still fires duplicate
   MSI-X if `bd-done-stub=on` is used for bisecting — making
   bisection results misleading. P2 makes `cb_complete` the sole
   MSI-X source structurally, not just by default config.
