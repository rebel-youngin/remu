...
| 2c — GIC `bpr>0` assertion | Immediately after `rebel_soft_reset`, NPU QEMU aborts with `arm_gicv3_cpuif.c:977: icc_gprio_mask: Assertion \`bpr > 0'`. The kmd's soft-reset doorbell write is the *first* host→NPU GIC SPI that ever fires — Stage 1 ran without any kmd, so the SPI 184/185 delivery path had never been exercised. Debugging hypothesis: CA73 CPU interface state at the moment of IRQ delivery has `ICC_BPR1_EL1 = 0`, which QEMU's GICv3 CPU model asserts against. Either (i) FreeRTOS hasn't programmed BPR yet at that point, (ii) our Group0/Group1 wiring picks the wrong group and reads an uninitialised BPR register, or (iii) this is a QEMU pre-existing bug that silicon would never hit because the IRQ arrives later on real hardware. | pending (blocker for Stage 3) |
| 3 — `TEST_IB` ring | Once Stage 2c clears, drive `kmd` ↔ `q-cp HIL` `TEST_IB` exchange on the shared-DRAM ring. | pending |
